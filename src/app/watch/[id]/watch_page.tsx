...
import { Calendar, Tv, Star, Download } from 'lucide-react';
...
export default async function WatchPage({
  ...
  // 5. Try to find the best print torrent on Nyaa
  let magnetUrl = '';
  if (bestGroup) {
    magnetUrl = await findEpisodeMagnet(bestGroup, title, currentEpNum);
  }
  // If no group-specific result, try a broader search
  if (!magnetUrl) {
    magnetUrl = await findEpisodeMagnet('', title, currentEpNum);
  }

  // 6. Fallback: Consumet HLS stream
  let streamUrl = '';
          ...
          <div className="mb-6 bg-primary/10 border border-primary/20 p-4 rounded-xl flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white text-sm font-medium">
                Recommended: <span className="text-primary">{seadex.bestRelease}</span>
              </p>
              {seadex.altRelease && (
                <p className="text-gray-500 text-xs mt-0.5">
                  Alt: {seadex.altRelease}
                </p>
              )}
            </div>
            {magnetUrl && (
